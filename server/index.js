...
const getLocations = async () => {
  ...
  try {
    const response = await axios.get(
      "https://api.openweathermap.org/geo/1.0/direct?q=Flemmingsberg&limit=5&appid=7285fa6b5bfe84a02e4ebf60e8efb9ce"
    );
...
const getWeather = async () => {
  ...
  try {
    ...
    const response = await axios.get(
      `https://api.openweathermap.org/data/2.5/weather?lat=${location.lat}&lon=${location.lon}&appid=7285fa6b5bfe84a02e4ebf60e8efb9ce`
    );
    return response.data;
  } catch (error) {
    console.error(error);
